...
contract AaveCLRobotOperatorTest is Test {
  ...
  address constant LINK_WHALE = 0x51268e001Ac2F53d3d22e2EE24E5af94fA2027ab;
  ILink constant LINK_TOKEN = ILink(0x404460C6A5EdE2D891e8297795264fDe62ADBB75);
  ...
  function _registerKeeper() internal returns (uint256, address) {
    vm.startPrank(LINK_WHALE);
    LINK_TOKEN.transfer(GovernanceV3BNB.EXECUTOR_LVL_1, 100 ether);
    vm.stopPrank();

    vm.startPrank(GovernanceV3BNB.EXECUTOR_LVL_1);
    LINK_TOKEN.approve(address(aaveCLRobotOperator), 100 ether);
    L2RobotKeeper ethRobotKeeper = new L2RobotKeeper(address(GovernanceV3BNB.EXECUTOR_LVL_1));
